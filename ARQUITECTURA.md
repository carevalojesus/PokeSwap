...
**Estado:** arquitectura acordada para el MVP el 11 de septiembre de 2026. La base React/Vite/TypeScript y el Worker Hono están implementados en la issue #1; el esquema D1, sus migraciones y pruebas de integridad se incorporan en #2. Las issues #3–#5 añaden catálogo, sorteo, registro y autenticación. La issue #6 incorpora navegación, componentes y Pokédex pública; la issue #7 conecta formularios, sesión y datos privados; #8 añade edición de perfil con control de versión y actualización de edad al cambiar el día en Lima. La issue #9 añade fotos privadas en R2, recorte y conciliación programada; ver [fotos implementadas](docs/FOTOS.md). La issue #10 incorpora colección agrupada, duplicados, reservas activas y progreso sobre 150; ver [colección implementada](docs/COLECCION.md). La issue #11 añade creación, consulta, cancelación y canje atómico de PokéDrops, con código recuperable mediante un secreto HMAC por entorno; ver [PokéDrops](docs/POKEDROPS.md). La issue #12 incorpora QR y enlaces de PokéDrop, con lector de cámara bajo demanda. Los intercambios siguen pendientes. Consultar el [README](README.md#documentación-y-ejecución) para comandos y alcance actual.
...
En PokéDrops se genera `/pokedrop#<código>` con qrcode.react 4.2.0; qr-scanner 1.4.2 se carga bajo demanda. El fragmento se retira de la barra de dirección al entrar y permanece solo en memoria durante login/registro; tras recargar hay que reabrir el enlace original. El servidor recibe el código exclusivamente por POST JSON y conserva las mismas validaciones de autenticación, rol, vigencia y límite de intentos que el ingreso manual. El lector acepta solo códigos válidos o enlaces del mismo origen y ruta exacta; nunca navega a direcciones leídas del QR. Escanear únicamente rellena el formulario.

La cámara se libera inmediatamente al cerrar, leer un código válido, consultar manualmente, salir de ruta, ocultar la pestaña o comprobar/cambiar sesión. También se libera un permiso concedido después de cerrar. Reanudar requiere pulsar de nuevo el botón.
